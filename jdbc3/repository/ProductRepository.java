...
public class ProductRepository {
    ...
    public static void setList(Product product){
        Connection con = JDBCConnector.getConnection();
        String sql = "INSERT into 제품 VALUES("+product.getProNum()+","+product.getProName()+","+product.getProStock()+","+product.getProPrice()+","+product.getProduceName()+")";
        PreparedStatement pstnt = null;
        try {
            pstnt.executeQuery(sql);
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
